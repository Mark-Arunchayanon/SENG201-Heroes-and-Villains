...
public class Hospital extends JPanel implements Location{
	...
	public void travelTo(Team team, boolean last_city) {
		...
		while (!items.isEmpty()) {
			...
			//User wants to return to Home Base
			if (selected_hero == null) {
				return;
			}
			
			//Provide the Healing items with the selected Hero's Healing stat
			for(HealingItem item : items) item.setHealing(selected_hero.getHealing());
